...
local bannner = require "user.config.banners"
local kind = require "user.config.kind"
local fortune = require "alpha.fortune"()

local default_header = {
  type = "text",
  val = bannner.dashboard(),
  opts = {
    hl = "Type",
    shrink_margin = false,
    position = "center",
...
local function file_button(fn, sc, short_fn, autocd)
  ...
  if fn_start ~= nil then table.insert(fb_hl, { "Comment", #ico_txt, #fn_start + #ico_txt }) end
...
local section = {
  ...
  mru = {
    ...
    val = {
      ...
      {
        type = "group",
        val = function() return { mru(10) } end,
      },
    },
  },
  mru_cwd = {
